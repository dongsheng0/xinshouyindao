...
const steps = [
  {
    ...
    content: (props: any) => (
      ...
      >
        <div>
          在产品首页点击”发起申请“按钮，进入发起用借章申请的流程(体验流程以用章为准)00
        </div>
      </TourContent>
    ),
  },
  {
    ...
    content: (props: any) => (
      <TourContent className="tour_01" total={steps.length} {...props}
        width="300"
        placement="top"
        position={{ left: '50px', bottom: '109px' }}>
        <div>
          发起流程过程中首先说明要办理什么业务，输入完毕后，点击进入下一环节01
        </div>
      </TourContent>
    ),
  },
  {
    ...
    content: (props: any) => (
      <TourContent className="tour_01" total={steps.length} {...props}
        width="200"
        placement="topright"
        position={{ right: '116px', bottom: '120px' }}>
  ...
  },
  {
    ...
    content: (props: any) => (
      <TourContent className="tour_01" total={steps.length} {...props}
        width="250"
        placement="top"
        position={{ left: '116px', bottom: '120px' }}>
  ...
  },
  {
    ...
    content: (props: any) => (
      <TourContent className="tour_01" total={steps.length} {...props}
        ...
        position={{ left: '116px', bottom: '120px' }}>
        <div>
          在发起前，用户可以确认一遍刚才填写的所有内容，确认无误后即可发布06
        </div>
      </TourContent>
    ),
  },
  {
    ...
    content: (props: any) => (
      <TourContent
        ...
        placement="bottom"
        width="300"
        position={{ left: '16px', top: '10px' }}
      >
        <div>
          申请发起之后，首先进入审批环节，审批人会收审批推送，点击即可查看07
        </div>
      </TourContent>
    ),
  },
  {
    ...
    content: (props: any) => (
      <TourContent
        ...
        placement="top"
        width="300"
        position={{ left: '50px', bottom: '120px' }}

      >
        <div>
          审批人可进行同意、拒绝和加签三种操作，点击快速完成全部审批环节08
        </div>
      </TourContent>
    ),
  },
  {
    ...
    content: (props: any) => (
      ...
      >
        <div>
          审批结束之后，印章管理员会收到推送，显示什么用户申请用什么章在什么文件时，点击即可查看9
        </div>
      </TourContent>
    ),
  },
  {
    ...
    content: (props: any) => (
      <TourContent className="tour_01" total={steps.length} {...props}
        placement="bottom"
        width="250"
        position={{ left: '116px', top: '682px' }}
      >
        <div>
          用章环节中，设计本申请的各个印章管理员要分别各自的章操作。点击此处，直接完成印章管理员盖章环节10
        </div>
      </TourContent>
    ),
  },
  {
    ...
    content: (props: any) => (
      ...
      >
        <div>
          印章管理员盖章完毕后，此时时机业务进行完毕，发起人就会手袋消息推送提示用户上传文件的盖章照片11
        </div>
      </TourContent>
    ),
  },
  {
    ...
    content: (props: any) => (
      <TourContent
        ...
        {...props}
        width="300"
        position={{ left: '36px', top: '670px' }}
      >
        <div>
          为满足业务闭环和文件错档等需要，申请完成后要求发起者将文件的盖章照片全部上传，点击直接完成照片的上传操作12
        </div>
      </TourContent>
    ),
  },
